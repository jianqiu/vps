...
// NewServer creates a new api soft layer VM pool server but does not configure it
func NewServer(api *operations.SoftLayerVMPoolAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
type Server struct {
	...
	api          *operations.SoftLayerVMPoolAPI
...
func (s *Server) SetAPI(api *operations.SoftLayerVMPoolAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving soft layer VM pool at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving soft layer VM pool at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		go func(l net.Listener) {
			...
			s.Logf("Stopped serving soft layer VM pool at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving soft layer VM pool at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving soft layer VM pool at https://%s", l.Addr())
